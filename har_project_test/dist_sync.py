...
def map_fun(args, ctx):
    ...
    logging.info(
        f" worker_num: {worker_num}, task_index: {task_index}, job_name: {job_name}, cluster_spec: {cluster_spec}")
    ...
    if job_name == "ps":
        ...
        with tf.device(tf.train.replica_device_setter(
            ...
            def define_optimizer(logits, labels):
                ...
                return train_step, accuracy, cross_entropy, global_step, prediction, sync_replicas_hook, chief_queue_runner

            x, y = define_placeholders()
            logits = build_graph(x)
            training_step, accuracy, cross_entropy_loss, global_step, pred, sync_replicas_hook, chief_queue_runner = define_optimizer(
                logits, y)
        ...
        sv.stop()
            ...
            # # The supervisor takes care of session initialization, restoring from
            # # a checkpoint, and closing when done or an error occurs.
            # # master=cluster_spec['ps'][0],
            # # init_op=init_op,
            # # summary_op=None,
            # # summary_writer=None,
            # # saver=saver,
            # # global_step=global_step,
            # logging.info("pre mon sess")
            # scaffold = tf.train.Scaffold(saver=saver, local_init_op=init_op)
            # with tf.train.MonitoredTrainingSession(master=server.target,is_chief=(task_index == 0),checkpoint_dir=logdir, hooks=[sync_replicas_hook]
            #                                        ) as mon_sess:
            #     logging.info("session ready, starting training")
            #
            #     # Loop until the supervisor shuts down or maximum steps have completed.
            ...
            #         logging.info("inside mon_sess loop")
            #         # Run a training step asynchronously.
            #         # See `tf.train.SyncReplicasOptimizer` for additional details on how to
            #         # perform *synchronous* training.
            #
            ...
            #         if len(batch_xs) > 0:
            #             if args.mode == "train":
            #                 mon_sess.run([training_step,global_step], feed_dict=feed)
            #                 # logging.info accuracy and save model checkpoint to HDFS every 100 steps
            #                 acc = mon_sess.run(accuracy, {x: batch_xs, y: batch_ys})
            #                 logging.info(f"step: acc: {acc}")
            #
            #                 # if mon_sess.is_chief:
            #                 # summary_writer.add_summary(summary, step)
            #             else:  # args.mode == "inference"
            #                 if (len(batch_ys == batch_size)):
            #                     preds, acc = mon_sess.run([pred, accuracy], feed_dict=feed)
            #                     # acc, loss = sess.run([accuracy, cross_entropy_loss], feed_dict=feed)
            #                     results = ["Label: {0}, Prediction: {1}".format(label, pred) for label, pred in
            #                                zip(batch_ys, preds)]
            #                     logging.info(f"len results: {len(results)}")
