...
def main_fun(argv, ctx):
    data_train_file = "../data/fashion_csv/fashion-mnist_train.csv"
    data_test_file = "../data/fashion_csv/fashion-mnist_test.csv"
    df_train = pd.read_csv(data_train_file)
    df_test = pd.read_csv(data_test_file)
    y_train = df_train.loc[:, ["label"]]
    x_train = df_train.loc[:, 'pixel1':'pixel784']
    y_test = df_test.loc[:, ["label"]]
    x_test = df_test.loc[:, 'pixel1':'pixel784']
    ...
    print('Number of train examples in dataset ' + str(len(df_train)))
    print('Number of test examples in dataset ' + str(len(df_test)))
    ...
    y_ = tf.placeholder(tf.int64, [None])  # correct answers(labels)
    ...
    cross_entropy_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y_, logits=logits, name='xentropy'))
    ...
    correct_predictions = tf.equal(tf.argmax(logits, 1), y_)
    ...
    def training_step(i, update_test_data, update_train_data):
        ...
        batch_X, batch_Y = x_train.sample(n=100).values, y_train.sample(n=100).values.reshape(100)
    # the backpropagation training step
        ...
        if update_test_data:
            test_acc, test_cos = sess.run([accuracy, cross_entropy_loss],
                                      feed_dict={x: x_test.values, y_: y_test.values.reshape(len(y_test.values))})
